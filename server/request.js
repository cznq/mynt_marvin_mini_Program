...
function get(requestRouter) {
  return checkRequestLogin('GET', requestRouter)
}
// POST请求
function post(requestRouter) {
  return checkRequestLogin('POST', requestRouter)
}
//检测是否已登陆
function checkRequestLogin(method, requestRouter) {
  return new Promise(function (resolve, reject) {
    ...
    app.checkWxLogin(function() {
      request(method, requestRouter, app).then(res=>{ resolve(res); })
    })
  })
}
...
// GET请求
function oldGET(requestHandler) {
  return CHECKRequest('GET', requestHandler)
}
// POST请求
function oldPost(requestHandler) {
  return CHECKRequest('POST', requestHandler)
}
//检测是否已登陆
function CHECKRequest(method, requestHandler) {
  return new Promise(function (resolve, reject) {
    ...
    if(requestHandler.params.method == 'login') {
      oldRequest(method, requestHandler, app).then(res=>{ resolve(res); })
    }else {
      app.checkWxLogin(function() {
        oldRequest(method, requestHandler, app).then(res=>{ resolve(res); })
      })
 ...
 */
function oldRequest(method, requestHandler, app) {
  return new Promise(function (resolve, reject) {
    ...
    wx.request({
      ...
      success: res => {
        ...
        resolve(res)//resolve(mockResponse.order)
      },
      fail: (res) => {
        app.myLog("请求错误", 'union_id:' + wx.getStorageSync('xy_session') + '\nopen_id:' + wx.getStorageSync('open_id') + '\n\n请求参数：\n' + JSON.stringify(requestHandler.params) + '\n\n接口返回信息：\n' + JSON.stringify(res))
        if (requestHandler.showLoading != false) {
